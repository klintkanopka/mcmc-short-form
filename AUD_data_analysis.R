...
test <- read.table("alcoholdata.txt", sep = ",", header = T)

y <- test$AUD
num_items <- ncol(test)-1
resp <- test[,2:c(num_items+1)]
...
# parallelized mcmc with a fixed (k) number of items
...
format_outpath <- function(output){
  
  out <- paste(c(as.character(output$k), 
                 as.character(round(output$q,2))), collapse = "_")
  
  outpath <- paste0("dataoutput/", out, ".RDS")
  
  return(outpath)
}

# loop through possible short forms, with different q values

for(k in 3:18){
  output1 <- mcmc_fixed_k(y, resp, k = k, iter = iter, q=0.5, nchains=5)
  saveRDS(output1, file = format_outpath(output1))

  output2 <- mcmc_fixed_k(y, resp, k = k, iter = iter, q=1/3, nchains=5)
  saveRDS(output2, file = format_outpath(output2))

  output3 <- mcmc_fixed_k(y, resp, k = k, iter = iter, q=2/3, nchains=5)
  saveRDS(output3, file = format_outpath(output3))
}

# combine results into a single dataframe
parse_result <- function(file){
  
  path <- paste0("dataoutput/", file)
  result <- readRDS(path)
...
results <- list.files("dataoutput/")
result_list <- lapply(results, parse_result)
final_result <- do.call(rbind, result_list)

#write_csv(final_result, file = "AUD_results.csv")

#final_result = read_csv("AUD_results.csv")

# plotting short form results
final_result$q <-  round(final_result$q, 3)
ggplot(final_result, aes(x = k, y = bw_accuracy, group = q, colour = as.factor(q))) + 
  geom_line() +
  ylab("Weighted balanced accuracy") + 
  xlab("Number of retained items")+
  labs(colour = "q weight") +
  scale_color_manual(values=c('0.333'='#FF0000',
                              '0.5'='#66BBDD',
                              '0.667' = '#9954E6'),
                     name = "q") +
  scale_fill_manual(values=c('0.333'='#FF0000',
                             '0.5'='#66BBDD',
                             '0.667' = '#9954E6'),
                    name = "q") +
  theme_minimal()
...
ggsave('fig/AUD_plot.png', height=5, width=6)


# comparing short forms with full form

# short form
final_result %>% group_by(q) %>% 
  summarize(max_bw = max(bw_accuracy),
            best_k = k[which.max(bw_accuracy)],
            my_cut = cut_score[which.max(bw_accuracy)]) 

results_sf <-  rbind(as.data.frame(final_result |> 
  filter(q == 0.333,
         k == 11)),
...
# full dataset
ss = rowSums(test[,2:21])
outcome = test[,1]

q = c(0.333, 0.5, 0.667)
q_results = matrix(NA, nrow = 21, ncol = 3)
for (i in 0:20){
  for(j in 1:3){
    q_results[i+1,j] = CalculateBalancedAccuracy(ss>i, outcome, q[j])
  }
}

apply(q_results, 2, which.max)

full_results_sf = as.data.frame(cbind(results_sf$bw_accuracy, results_sf$k, results_sf$cut_score,
                        rbind(q_results[6,1], q_results[6,2], q_results[5,3]),rbind(5,5,4), q))
names(full_results_sf) = c("short BW", "short k", "short cut", "full BW", "full cut", "q")

round(full_results_sf, 3)
